...
> Predication is a relation involving a fundamental logical difference between its two terms. Predicates may themselves have predicates, but the predicates of predicates will be radically different from the predicates of substances. (1911: 23)\[[7](notes.html#note-7)\]  
...
Here are some candidate answers to the SCQ. Contact: ‘the *x*s are in contact’. Life: ‘the *x*s are caught up in a life’. Mereological Nihilism: ‘there is only one of the *x*s’. Mereological Universalism: ‘the *x*s are one or more in number’.  
...
#### 3.2.2 Relational Ontologies 关系本体
...
In contrast to the constituent ontologies surveyed above, relational ontologies posit no internal structure to objects beyond ordinary mereological structure. Objects are not layer-cakes of thin particulars and properties (or layer-cakes of matter and form); they are blobs instead. Put this way, the blob view of objects has only _negative_ content; it tells us what objects are not. It offers, then, a partial account of the nature of objects, and thus invites supplementation.
与上述讨论的组成本体论不同，关系本体论认为对象除了普通的部分整体结构外没有内部结构。对象不是由细小的具体事物和属性（或物质和形式）组成的多层蛋糕；相反，它们更像是“blob”（无定形的实体）。这样说来，对象的“blob”观点仅具有否定性内容；它告诉我们对象不是什么。因此，它对物体的性质提供了一个部分解释，从而需要进一步补充。

We will now consider two relational ontologies and their implications for the theory of objects.
我们现在将考虑两种关系本体论及其对对象理论的影响。

##### Platonism 柏拉图哲学

The Platonist view of properties is that they are transcendent universals. They exist independently, and are outside of space and time, unchanging, and causally inert. Clearly such things cannot be parts of ordinary material objects which are inside space and time, change, and participate in causal chains. But objects still are red, large, heavy, and the like—and this in virtue of standing in some relation to the transcendent universals. That relation is usually called _exemplification_ or _instantiation_, and it is taken as primitive. But, importantly, it is _external_—objects are related to things outside of themselves—whereas the relation between object and property on constituent ontologies is _internal_.  
柏拉图主义对属性的看法是，它们是超越的普遍性。它们独立存在，在空间和时间之外，不变，并且因果惰性。显然，这些事物不可能是普通物质对象的一部分，这些物质对象在空间和时间内部，变化并参与因果链。但物体仍然是红色的、大的、沉重的等等——这是由于与超然的普遍性有某种关系。这种关系通常称为 exemimplification 或 instantiation，它被视为原始关系。但是，重要的是，它是外部的——对象与自身之外的事物相关——而组成本体论上的对象和属性之间的关系是内部的。
...
因此，虽然柏拉图主义主要是一种关于属性本质的观点，但它对物体的本质也有影响。特别是，它意味着它们没有作为 parts 的属性。正如阿姆斯特朗所说：
...
> 有趣的是，普遍性的分离领域理论允许一个 blob，而不是对细节的层蛋糕视图。因为从这个角度来看，一个事物拥有属性是什么？它不是事物具有某种内在特征，而是它与另一个领域的某些普遍性或形式有一种关系，即实例化关系。事物本身可能是斑点状的。（阿姆斯特朗 1989：76-77）
...
事实证明，柏拉图主义并不是唯一一种与非结构化或斑点状对象视图相适应的属性解释。

##### Class Nominalism 类名词主义

On class nominalism, properties are classes of things or classes of possible things. The property _being green_ is the class of all green things, the property _being wise_ is the class of all wise things, the relation _is the sister of_ is the class of ordered pairs the first member of which is the sister of the second member, and so on. \[[35](notes.html#note-35)\]  
在类名词论中，属性是事物的类或可能事物的类。属性 being green 是所有绿色事物的类，属性 being wise 是所有智慧事物的类，关系是 的姐妹是有序对的类，其中的第一个成员是第二个成员的姐妹，依此类推。\[35\]
...
在这个观点中，一个对象具有（或示例化，或实例化，或其他任何）属性，就是它属于一类事物。类成员身份和集合成员身份一样，是一种外部关系;所以类名词最适合对象的 blob 视图。van Inwagen 对这种联系的解释如下：
...
> 根据 Lewis \[类名词论者\] 的说法，属性是一组可能的对象。（当且仅当某物是一个集合时，它就是属性，其所有成员都是可能的对象。刘易斯说，猪或猪的属性就是所有可能的猪的集合——一个比实际猪的集合大得多的集合。考虑一只真正的猪 Freddy。Freddy 当然有 porcinity。猪和财产之间的这种 “拥有” 的关系是什么？为什么，简单地设置成员资格。而一组可能性与其个体成员的关系当然不是选区。从某种意义上说，Freddy 无疑是所有可能的猪的集合的组成部分——“成分”是一个非常灵活的词，它可能足够灵活以允许这种应用——但是在某种意义上，所有可能的猪的集合是 Freddy 的一个组成部分，这是不可想象的。(2011: 392–393)
